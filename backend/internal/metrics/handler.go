...
func (h *Handler) EscalatedBlockers(c *gin.Context) {
	cycleID := c.Query("cycle_id")
	if cycleID == "" {
		cycleID = "a0000000-0000-0000-0000-000000000001"
	}
	...
	var reports []models.Report
	h.db.Preload("User").
		Where("cycle_id = ? AND blockers_content <> ''", parsedCycleID).
		Find(&reports)

	var result []BlockerItem = []BlockerItem{}
	for _, r := range reports {
		owner := r.User.FirstName + " " + r.User.LastName
		department := r.User.Department

		reference := r.CreatedAt
		if r.SubmittedAt != nil {
			reference = *r.SubmittedAt
		}
		daysActive := int(time.Since(reference).Hours() / 24)
		if daysActive < 0 {
			daysActive = 0
		}

		text := strings.TrimSpace(r.BlockersContent)
		title := text
		...
		result = append(result, BlockerItem{
			ID:          r.ID.String(),
			...
			IsCritical:  strings.Contains(strings.ToUpper(text), "CRITICAL"),
		})
	}
...
func (h *Handler) CycleHistory(c *gin.Context) {
	...
	for _, cycle := range cycles {
		...
		compliance := 0.0
		if totalEmployees > 0 {
			compliance = float64(submitted) / float64(totalEmployees) * 100
		}
